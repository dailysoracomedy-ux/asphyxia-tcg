...
def grime():
    ...
    a = np.clip((0.55 - n) / 0.55, 0, 1) ** 1.4
    # near-black, denser - Commit 46.1: the first cut read too light/washed
    save_la(np.full((512, 512), 4), a * 135, 'grime.png')
...
def scratches():
    ...
    for _ in range(70):
        ...
        a[ys, xs] = rng.uniform(28, 80)  # 46.1: halved - screen layer was washing panels
    img = Image.fromarray(a.astype(np.uint8)).filter(ImageFilter.GaussianBlur(0.4))
    save_la(np.full((512, 512), 200), np.asarray(img), 'scratches.png')


def plate():
    """512px dark brushed-metal plate: base tone + horizontal brushing +
    low-frequency mottling. Opaque - panel base surface."""
    base = octave_noise(512, octaves=(3, 6), weights=[1, 0.5]) * 22 + 7  # 46.1: darker
    ...
    lum = make_tileable(np.clip(base + brushed * 1.0, 4, 42))
...
def frame():
    ...
    band = 16 + n * 18            # 46.1: thinner band, 16..34px - was grazing content
    ...
    a[inside_band] = (120 + n[inside_band] * 80) * fall[inside_band]
