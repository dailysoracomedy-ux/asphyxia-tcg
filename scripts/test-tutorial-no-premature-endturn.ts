...
async function main() {
  ...
  // This scenario specifically tests the empty-board case ("no Apex can
  // still attack" because there's no Apex at all) - clear the board right
  // after opening selection (which is required to reach 'playing' status)
  // so it's genuinely empty, not just about to have an Apex placed on it.
  useGameStore.setState((st) => ({
    players: { ...st.players, player1: { ...st.players.player1, apexSlots: [null, null] } },
  }));
  s = useGameStore.getState();
  if (s.phase === 'Start' && s.startPhasePending) s.advancePhase('Start');
  s = useGameStore.getState();
