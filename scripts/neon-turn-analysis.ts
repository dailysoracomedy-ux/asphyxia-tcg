...
function runOneGame(neonId: PlayerId, oppFaction: Faction, maxTurns: number, o2Amount: number): { winner: PlayerId | null; neonId: PlayerId; snapshots: TurnSnapshot[] } {
  const s = useGameStore.getState();
  s.resetToMenu();
  const f1: Faction = neonId === 'player1' ? 'Neon Underground' : oppFaction;
  const f2: Faction = neonId === 'player1' ? oppFaction : 'Neon Underground';
  ...
  const oppId: PlayerId = neonId === 'player1' ? 'player2' : 'player1';
  ...
  while (useGameStore.getState().status === 'playing' && turns < maxTurns && guardTop < maxTurns * 30) {
    ...
    if (st.phase === 'Combat') {
      ...
      turns += 1;
      snapshots.push(snapshot(turns, neonId, oppId));
  ...
  return { winner: finalState.winnerId, neonId, snapshots };
}

const O2_LEVELS = [24, 48, 96];
const OPPONENTS: Faction[] = ['Dark White', 'Synth Ascendancy'];
...
function main() {
  ...
  for (const o2 of O2_LEVELS) {
    ...
    for (const opp of OPPONENTS) {
      for (let i = 0; i < GAMES_PER_CELL; i++) {
        ...
        const neonId: PlayerId = i % 2 === 0 ? 'player1' : 'player2';
        gamesRun += 1;
        try {
          const { winner, snapshots } = runOneGame(neonId, opp, maxTurns, o2);
          const neonWon = winner === neonId;
          ...
          crashed += 1;
          console.error(`\n!!! CRASHED: O2=${o2} Neon(${neonId}) vs ${opp} [run ${i}] !!!`);
          console.error(e);
        }
      }
    }
  ...
  for (const o2 of O2_LEVELS) {
    console.log(`--- O2=${o2} (all Neon vs {Dark White, Synth Ascendancy} games) ---`);
