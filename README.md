...
into `public/` automatically by `scripts/sync-static.mjs` (predev/prebuild
hooks in package.json - `npm run dev` and `npm run build` both sync first).
Served URLs are unchanged; the synced copies in `public/art` + `public/images`
are gitignored - **static2/ is the source of truth**. Upload `public/` (65
files) and `static2/` (49) as separate folders; when either nears 100, add a
`static3/` and one line to SYNC_DIRS.

**Commit 47.1** — the four main-menu buttons are hand-made plates (New Game /
Learn To Play / Simulated Match / The Locker, `public/ui/menu-*.webp`, labels
baked in with sr-only text preserved, glow hovers kept); the menu card is now
flat opaque black (no transparency, no panel texture).

**Commit 47 — hand-made UI art, the full-bleed coin stage, and particle systems.**

- **User-provided art wired in** (`public/ui/`, 4x sources served at 2x webp,
  ~360KB total): bespoke buttons with baked labels (deck picker per faction,
  O2 amounts, START, HEADS/TAILS, END TURN, CONTINUE in the tutorial) via the
  new `.btn-art` treatment - real labels stay in the DOM as sr-only for
  accessibility and tests; glow shadows, hover/press physics and sounds are
  all preserved. Containers resized to each artwork's aspect ratio (per
  direction) rather than stretching art to old boxes.
- **Universal frame & panel**: every remaining `.btn-3d` button wears the
  provided universal button face via a 9-slice `border-image ... fill` on
  `::before` - painted above backgrounds but **below text**, so labels can
  never be covered; panels use the provided panel art the same way. The
  procedural grunge textures are superseded on chrome (still used by the
  world grain + stencil tags).
- **Per-faction stat plates**: the sidebar player chips use the provided
  255x60 plates, boxes fixed so art never stretches, typography tightened to
  fit inside the art's border.
- **Deck-select screen**: version pill + audio sliders removed, logo at 80%.
- **Coin stage**: canvas now runs edge-to-edge on the menu card (446x600),
  ending the apex/glow clipping for good.
- **Particles**: Engines short-circuit - an erratic electric spark burst +
  arc-flicker when an Engine is played or triggers; destroyed cards throw
  dark debris chunks + faction embers with gravity; cards landing on slots
  puff a dust ring. All reduced-motion aware.
- Repo rule now in force: no folder over 100 files (all current folders are
  well under; new asset namespaces get sibling folders when needed).

**Commit 46 — THE GRUNGE PASS: the UI becomes worn hardware in a living world.**

- **Procedural grunge texture kit** (`scripts/generate-grunge-textures.py` →
  `public/textures/`, ~210KB total): film grain, tileable grime blotches, wear
  scratches, brushed dark plate, and a 9-slice chipped frame. All generated
  (deterministic seed) since the build environment can't fetch texture packs.
- **Chrome restyled in place**: `.btn-3d` / `.panel-3d` / `.panel-3d-deep`
  keep their names and press/elevation mechanics, so every Commit 45 call
  site transformed at once. Buttons wear grime and heavier shading with text
  kept fully clean (46.1: the chipped frame was removed from buttons - small
  buttons' padding is thinner than the frame band, so chips landed on the
  letters); panels are dark grimy brushed-metal plates with sheen and thin
  chipped borders in two elevation tiers (46.1: darker plate/grime, the
  lightening scratch layer dropped from standard panels, grain switched to
  dim soft-light - the first cut read too light/washed).
- **The world is alive**: animated film grain over the entire scene (board,
  chrome and modals share one grade); the active player's faction nameplate
  is a spray-stencil tag that flickers like cheap neon; critical O2 (<=6)
  pulses the readout like an alarm; three momentum pips hum beneath each
  player's vitals. All reduced-motion aware. Stat components (O2Stat/
  MomentumStat) untouched - the life is layered around their drop-zone logic.

**Commit 45 — full-bleed sleeves, deck boxes scrapped, and the 3D chrome pass.**

- **Sleeves own the deck now**: deck boxes are removed entirely (registry,
  store field, Locker tab, rendering) - both deck and void render as full-bleed
  sleeved card-back stacks, so the equipped sleeve covers the entire card. A
  stale `deckbox` key in persisted localStorage is harmless (nothing reads it).
- **Real sleeve designs**: `SleeveSkin` gains an optional printed `overlay`
  layer (CSS composited over the art, with blend modes), turning tints into
  designs. Five new sleeves join the five tints: Blood Oath (hazard scoring),
  Circuit Gold (gilded grid), Void Static (scanlines + vignette), Holo Rift
  (iridescent laminate), Hazard Line (warning striping). Overlays render on
  piles and in Locker previews from the same registry data.
- **3D chrome everywhere**: new `.btn-3d` / `.panel-3d` / `.panel-3d-deep`
  treatments (globals.css) applied across the whole UI - main menu card and
  MenuButtons, faction/O2 pickers, HEADS/TAILS, End Turn, Engine Reconfig,
  attack pickers, OPTIONS dropdown, sidebar chips, Rift/momentum panels, all
  modals, banners, tutorial panel, Locker. Buttons are convex (lit top face,
  shadowed underside), lift on hover, and genuinely PRESS on click (travel
  down + inverted inset shadow). Panels get bevel + elevation in two tiers.
  All consistent with the unified top-left light convention.
